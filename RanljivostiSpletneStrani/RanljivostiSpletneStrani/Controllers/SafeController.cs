...
namespace RanljivostiSpletneStrani.Controllers
{
    public class SafeController : Controller
    {
        ...
        {
            //   dobijo id trenutnega
            var sessionUserId = HttpContext.Session.GetInt32("MyId");

            if (sessionUserId == null)
            {
                // Če še nihče ni prijavljen, prvi kliknjen ID postane "naš"
                HttpContext.Session.SetInt32("MyId", id);
                sessionUserId = id;
            }

            // ce trenutnu se ujema z zahtevanim
            if (id != sessionUserId)
            {
                return Unauthorized($"Dostop zavrnjen! Prijavljeni ste kot ID {sessionUserId}, zato ne smete gledati profila ID {id}.");
            }
