...
namespace Sifriranje
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private byte[] _aesIV;  // Tukaj bo shranjen initializacijski vektor
        ...
        {
            int izbranaDolzina = int.Parse(((ComboBoxItem)cbAesSize.SelectedItem).Content.ToString()); //preberemo izbiro dolzine  
        ...
        {
            int rsaSize = int.Parse(((ComboBoxItem)cbRsaSize.SelectedItem).Content.ToString());  //izbira dolzine kljuca
        ...
        private void BtnSaveKeys_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (RSA rsa = RSA.Create())
            {
                ...
                encryptedAesKey = rsa.Encrypt(_aesKey, RSAEncryptionPadding.OaepSHA256);  // enkriptamo  z rsa javnim kljucem
            }
            ...
            if (sfd.ShowDialog() == true)
            {
                using (BinaryWriter writer = new BinaryWriter(File.Open(sfd.FileName, FileMode.Create)))
                {
                    ...
                    {
                        rsaForExport.ImportParameters(_rsaPrivateKey);  //privat kljuc
        ...
        private void BtnLoadKeys_Clicl(object sender, RoutedEventArgs e)
        {
            ...
            if (ofd.ShowDialog() == true)
            {
                try
                {
                    ...
                    {
                        // preberemo aes kljuc
                        int encryptedKeyLength = reader.ReadInt32();   //koliko bajtev ima
                        byte[] encryptedAesKey = reader.ReadBytes(encryptedKeyLength);   // prebere toliko bajtev kot je dolg kljuc
